...
用于 EviDTI 模型的训练，支持 DTI 分类和 DTA 回归任务。
...
def dirichlet_loss(y_true, alphas, lam=1):
    """
    使用 Evidential Learning Dirichlet 损失函数 (Sensoy et al.)
    
    Args:
        y_true: 真实标签 (one-hot 编码)
        alphas: 模型输出的 alpha 参数
        lam: KL 正则化项的权重系数
    ...
    def KL(alpha):
        """
        计算 Dirichlet 分布与均匀 Dirichlet 分布之间的 KL 散度
        """
    ...
    # 确保 y_true 是 one-hot 格式
    y_one_hot = y_true
    if y_one_hot.dim() == 1:
        # 如果是类别标签，转换为 one-hot
        num_classes = alphas.shape[-1]
        y_one_hot = torch.zeros(alphas.shape[0], num_classes, device=alphas.device)
        y_one_hot.scatter_(1, y_true.long().unsqueeze(1), 1)

    # SOS (Sum of Squares) 项
...
class EviDTI_Trainer(BaseTrainer):
    """
    EviDTI 训练器
    ...
    def __init__(self, args, logger, dataset, model, optimizer):
        ...
        self.evidential = getattr(model, 'evidential', False)
        self.lam = 1.0
        ...
        for inp in self.dataset.train_loader:
            ...
            if self.task_type == 'classification':
                if self.evidential:
                    # 证据感知学习的损失计算
                    # 将标签转换为 one-hot 编码
                    labels_onehot = torch.zeros(logits.size(0), 2).to(logits.device)
                    labels_onehot.scatter_(1, labels.unsqueeze(1), 1)
                    loss = dirichlet_loss(labels_onehot.float(), logits, self.lam)
                else:
            ...
            if self.task_type == 'classification':
                ...
                # 从 alpha 参数计算概率
                probs = logits[:, 1] / logits.sum(dim=1)
    ...
    def _evaluate(self, loader):
        ...
        if self.task_type == 'classification':
            ...
            # 从 alpha 参数计算概率
            y_prob_np = y_logits[:, 1].numpy() / (y_logits.sum(dim=1).numpy() + 1e-8)
    ...
    def train(self):
        ...
        evi_str = " (证据感知学习)" if self.evidential else ""
        print("=" * 60)
        print(f"开始 EviDTI 训练 ({task_name}){evi_str}")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print(f"EviDTI 训练完成 ({task_name}){evi_str} - 最终测试结果")
        print("=" * 60)
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'EviDTI')}\n")
