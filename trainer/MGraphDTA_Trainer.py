...
用于 MGraphDTA 模型的训练，支持 DTI 分类和 DTA 回归任务

特点:
1. 自适应损失函数：根据任务类型选择 CrossEntropyLoss 或 MSELoss
2. 支持多种评估指标
3. 模型变体支持：embedding (使用预训练嵌入), graph (原始图神经网络)
"""
...
class MGraphDTA_Trainer(BaseTrainer):
    """
    MGraphDTA 训练器
    ...
    def train(self):
        ...
        print("=" * 60)
        print(f"开始 MGraphDTA 训练 ({task_name})")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print(f"MGraphDTA 训练完成 ({task_name}) - 最终测试结果")
        print("=" * 60)
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'MGraphDTA')}\n")
            ...
            f.write(f"  variant: {getattr(self.args, 'mgraphdta_variant', 'embedding')}\n\n")
