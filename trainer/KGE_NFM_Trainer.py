"""
KGE_NFM (Knowledge Graph Embedding + Neural Factorization Machine) Trainer

用于 KGE-NFM 模型的训练，支持 DTI 分类和 DTA 回归任务

特点:
1. 自适应损失函数：根据任务类型选择 CrossEntropyLoss 或 MSELoss
2. 支持多种评估指标
3. 模型变体支持：standard, bilinear, attention
"""
...
class KGE_NFM_Trainer(BaseTrainer):
    """
    KGE-NFM 训练器
    ...
    - 评估指标: Accuracy, Precision, Recall, F1, AUC, AP (分类) / MSE, MAE, RMSE, R2 (回归)
    """
    ...
    def _prepare_batch_data(self, inp, task_type=None):
        ...
        Args:
            inp: 输入数据 (drug_idx, protein_idx, labels)
    ...
    def _evaluate(self, loader):
        ...
        if self.task_type == 'classification':
            ...
            y_pred_np = y_logits.squeeze(-1).numpy()  # [N, 1] -> [N]
    ...
    def train(self):
        ...
        print("=" * 60)
        print(f"开始 KGE-NFM 训练 ({task_name})")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print(f"KGE-NFM 训练完成 ({task_name}) - 最终测试结果")
        print("=" * 60)
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'KGE_NFM')}\n")
            ...
            f.write(f"  variant: {getattr(self.args, 'kge_nfm_variant', 'standard')}\n\n")
