...
用于 RSGCL_DTI 模型的训练，支持 DTI 分类和 DTA 回归任务。
...
class RSGCL_DTI_Trainer(BaseTrainer):
    """
    RSGCL_DTI 训练器
    ...
    def train(self):
        ...
        print("=" * 60)
        print(f"开始 RSGCL_DTI 训练 ({task_name})")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print(f"RSGCL_DTI 训练完成 ({task_name}) - 最终测试结果")
        print("=" * 60)
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'RSGCL_DTI')}\n")
