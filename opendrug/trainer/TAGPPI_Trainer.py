...
class TAGPPI_Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch, task_type=None):
        ...
        for inp in self.dataset.train_loader:
            ...
            if self.task_type == 'multilabel':
                ...
                ypreds.append(preds)
                yprobs.append(probs)
                ys.append(labels.detach().cpu().numpy().flatten())
        ...
        if self.task_type == 'multilabel':
            metrics = self._compute_metrics_multilabel(y_true, y_pred, y_prob)
        else:
    ...
    def train(self):
        task_desc = 'TAGPPI 二分类' if self.task_type == 'binary' else 'TAGPPI 多标签'
        print("=" * 60)
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                if self.task_type == 'binary':
                    ...
                    print(f", F1-micro: {train_metrics['F1-micro']:.4f}, "
                          ...
                          f"AUC-ML-micro: {train_metrics['AUC-ML-micro']:.4f}")
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'TAGPPI')}\n")
