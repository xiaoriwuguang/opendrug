...
用于 MMD_DTA 模型的训练，支持 DTI 分类和 DTA 回归任务。
...
class MMD_DTA_Trainer(BaseTrainer):
    """
    MMD_DTA 训练器
    ...
    def train(self):
        ...
        print("=" * 60)
        print(f"开始 MMD_DTA 训练 ({task_name})")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print(f"MMD_DTA 训练完成 ({task_name}) - 最终测试结果")
        print("=" * 60)
    ...
    def _save_results(self, final_metrics, best_epoch):
        ...
        with open(out_file, 'a') as f:
            ...
            f.write(f"  model: {getattr(self.args, 'model', 'MMD_DTA')}\n")
