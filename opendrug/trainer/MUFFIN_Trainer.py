...
class MUFFIN_Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch: int, scaler: torch.cuda.amp.GradScaler,
        ...
        # Pre-move embeddings to device once per epoch (or could be done in init)
        entity_embed = self.dataset.entity_pre_embed.to(self.device)
        structure_embed = self.dataset.structure_pre_embed.to(self.device)

        for inp in self.dataset.train_loader:
            # inp from collate_fn is [None, None, edge_index_cpu, labels_cpu]
            edge_index = inp[2].to(self.device)
            labels_raw = inp[3]
            
            if task_type == 'multiclass':
                labels = labels_raw.to(self.device, dtype=torch.long)
            else:
                labels = labels_raw.to(self.device, dtype=torch.float32)

            # Reconstruct input with GPU embeddings
